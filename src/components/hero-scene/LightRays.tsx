...
export interface LightRaysProps {
  ...
  lightMode?: boolean;
  className?: string;
  /** Normalized 0–1 aim point when not using the mouse yet (e.g. title block). */
...
export function LightRays({
  ...
  lightMode = false,
  className = "",
  initialMousePosition = { x: 0.5, y: 0.5 },
}: LightRaysProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const uniformsRef = useRef<Uniforms | null>(null);
  const rendererRef = useRef<Renderer | null>(null);
  const mouseRef = useRef(initialMousePosition);
  const smoothMouseRef = useRef(initialMousePosition);
  ...
  useEffect(() => {
    mouseRef.current = initialMousePosition;
    smoothMouseRef.current = initialMousePosition;
...
void main() {
  ...
  gl_FragColor = color;
      ...
      const uniforms: Uniforms = {
        ...
        mousePos: { value: [initialMousePosition.x, initialMousePosition.y] },
  ...
  useEffect(() => {
    ...
    uniforms.lightMode.value = lightMode ? 1.0 : 0.0;
